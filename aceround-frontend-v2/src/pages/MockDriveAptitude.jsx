...
// Round 0 of the Mock Drive pipeline — the very first round, before MCQ.
// Generic Quantitative + Logical Reasoning + Verbal Ability mix (not tied
// to role/resume), mirroring how real campus placement drives (TCS NQT,
// Infosys, Wipro) always start with an Aptitude round before the technical
// rounds. Pattern is intentionally the same as MockDriveMcq.jsx.
const MockDriveAptitude = () => {
  const { id } = useParams();
  ...
  const submitAnswers = useCallback(async (finalAnswers) => {
    ...
    setError('');
    try {
      const data = await mockDriveApi.submitAptitude(id, finalAnswers);
      setResult(data);
    } catch (err) {
  ...
  const loadQuestions = async () => {
    ...
    clearInterval(timerRef.current);
    try {
      const data = await mockDriveApi.startAptitude(id);
      setQuestions(data.questions || []);
      setTimeLeft(data.timeLimitSeconds || (data.questions || []).length * 45);
    } catch (err) {
      setError(err.message || 'Could not load aptitude questions.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0a0f1e] text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10 sm:py-14">
        {!loading && !result && questions.length > 0 && (
          <div className="sticky top-16 z-30 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 mb-6 bg-[#0a0f1e]/95 backdrop-blur-xl border-b border-slate-800/60 flex items-center justify-end">
            <div
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Aptitude Round</h1>
          <p className="text-slate-400 text-sm sm:text-base">
            A mix of Quantitative, Logical Reasoning, and Verbal Ability questions — just like a
            real campus placement drive. Score 60% or higher to unlock the MCQ round.
          </p>
        </div>
        ...
        {result && (
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 sm:p-8 text-center">
            ...
            {result.passed ? (
              <button
                type="button"
                onClick={() => navigate(`/mock-drive/${id}/mcq`)}
...
export default MockDriveAptitude;
